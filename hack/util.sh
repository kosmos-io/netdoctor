...
#https://textkool.com/en/ascii-art-generator?hl=default&vl=default&font=DOS%20Rebel&text=NetDoctor
NetDoctor_GREETING='
--------------------------------------------------------------------------------------
 ██████   █████           █████    ██████████                      █████
░░██████ ░░███           ░░███    ░░███░░░░███                    ░░███
...
function util::install_environment_check {
    ...
    echo "Sorry, NetDoctor installation does not support $ARCH/$OS at the moment"
    exit 1
}
...
function util::version_ldflags() {
  ...
  LDFLAGS="-X github.com/kosmos.io/netdoctor/pkg/version.gitVersion=${GIT_VERSION} \
                        -X github.com/kosmos.io/netdoctor/pkg/version.gitCommit=${GIT_COMMIT_HASH} \
                        -X github.com/kosmos.io/netdoctor/pkg/version.gitTreeState=${GIT_TREESTATE} \
                        -X github.com/kosmos.io/netdoctor/pkg/version.buildDate=${BUILDDATE}"
  echo $LDFLAGS
}
